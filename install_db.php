...
if(SQL_TYPE == 'pgsql') {
	sql_action("CREATE TABLE feeds (id serial, url varchar, email varchar, avatar varchar, approved int, reg_date date);");
	sql_action("CREATE TABLE users (id serial, email varchar, pass varchar(32), name varchar, role_level varchar);");
	sql_action("CREATE TABLE settings (id serial, name varchar, value text);");
} else if(SQL_TYPE == 'mysql') {
// 	sql_action("DROP TABLE feeds;");
// 	sql_action("DROP TABLE users;");
// 	sql_action("DROP TABLE settings;");
	sleep(1);
	sql_action('CREATE TABLE `feeds` ( `id` INT NOT NULL AUTO_INCREMENT PRIMARY KEY, `url` VARCHAR(255) NOT NULL, `email` VARCHAR(255) NOT NULL, `avatar` VARCHAR(255) NOT NULL, `approved` INT(1) NOT NULL, `reg_date` DATE, UNIQUE (`url`) ) ENGINE = myisam;');
	sql_action('CREATE TABLE `users` ( `id` INT NOT NULL AUTO_INCREMENT PRIMARY KEY, `email` VARCHAR(255) NOT NULL, `pass` VARCHAR(32) NOT NULL, `name` VARCHAR(255) NOT NULL, `role_level` VARCHAR(255) NOT NULL, UNIQUE (`name`) ) ENGINE = myisam;');
	sql_action('CREATE TABLE `settings` ( `id` INT NOT NULL AUTO_INCREMENT PRIMARY KEY, `name` VARCHAR(255) NOT NULL, `value` TEXT, UNIQUE (`name`) ) ENGINE = myisam;');
}

sql_action("INSERT INTO feeds VALUES (".sql_autoid('feeds').", 'http://josiplisec.net/feed', 'josiplisec@gmail.com', 'http://josiplisec.net/files/planetoid_avatar.png', 1, '".date('Y-m-d')."');");
